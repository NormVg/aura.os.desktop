  ...
  const state = reactive({
    ...
    appearance: {
      ...
      ai: {
        ...
        systemPrompt: 'You are Aura, a helpful AI assistant embedded in Aura OS — a creative desktop environment. Be concise, warm, and helpful. You can include emotion hints like [emotion:thinking], [emotion:happy], [emotion:surprised] in your responses to express your emotional state.',
      },
      about: {
        version: '1.0.0',
      }
    ...
    async function loadState() {
      ...
      if (saved && saved.settings) {
        ...
        if (saved.settings.appearance) state.appearance = { ...state.appearance, ...saved.settings.appearance }
        if (saved.settings.ai) {
          ...
          if (saved.settings.ai.roles) state.ai.roles = { ...state.ai.roles, ...saved.settings.ai.roles }
